...
public class GapWebViewClient extends WebViewClient {
	...
	private static final String TAG="GapViewClient";
	 ...
	 * @param ctx
	 */
	public GapWebViewClient(GapView ctx) {
		this.ctx = ctx;
	}
	...
	public boolean shouldOverrideUrlLoading(WebView view, String url) {
		...
		// All else
			...
			// redisplayed.
			if (url.startsWith("file://") || url.indexOf(this.ctx.getBaseUrl()) == 0
	...
	public void onPageFinished(WebView view, String url) {
		Log.i("GapWebViewClient", "onPageFinished url: "+url);
		...
		if (!url.equals("about:blank")) {
			ctx.loadUrl("javascript:try{ PhoneGap.onNativeReady.fire();}catch(e){_nativeReady = true;}");
		}

		// Make app visible after 2 sec in case there was a JS error and
		// PhoneGap JS never initialized correctly
		if (ctx.getVisibility() == View.INVISIBLE) {
